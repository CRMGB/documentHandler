...
class UploadView(TemplateView):
        ...
        try:
            ...
            for row in DictReader(rows):
                ...
                if not is_valid_uuid(row.get("book_id")):
                    messages.error(
                        request, f"The book with the id '{row['book_id']}' is wrong."
                    )
                    return redirect("csv_upload")
            ...
            form.save()
            messages.success(request, f"File successfully uploaded! {file.name}")
            table = self.generate_table(request)
            context = {"form": CSVFileRowsForm(), "table": table}
            return render(request, "csv_upload.html", context)
        except AttributeError as error:
    ...
    def create_file_and_send_to_s3(
        ...
        file_model = CSVFileModel.objects.create(
            file_name=csv_file.name, user=self.request.user, row_count=row_count
        )
...
class PagedFilteredTableView(SingleTableView):
    ...
    def get_queryset(self, **kwargs):
        ...
        if CSVFileModel.objects.filter(id=id):
            qs = (
                super(PagedFilteredTableView, self)
                .get_queryset()
                .filter(file__id=self.kwargs["pk"])
            )
